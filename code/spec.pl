...
match(_event, new_et(HashId)) :- deep_subdict(_event, _{event:"init_post", class:"HashSet", res:HashId}).
...
trace_expression('Main', Main) :- (Main=(star(not_new_et)*optional(var(hashId, (new_et(var(hashId))*(app(VerifyHash, [var(hashId)])/\Main)))))),
