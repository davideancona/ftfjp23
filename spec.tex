...
\paragraph{Possible generalization of the specification}
...
\subparagraph{Classes based on hash tables:} \lstinline{HashMap} is a widely used class of the Java API. To consider also this class,
...
\subparagraph{Mutable classes redefining \lstinline{hashCode}:} 
...
\lstinline{set(index,elem)} of \lstinline{List}, for which it is not easy to test whether the target object is modified. For instance, the method
may replace an element in a list with the same object, or an equal object. In that case, the verification may detect more false positives.
...
The \lstinline{put} method above has similar problems as method \lstinline{set}, hence the verification may be less accurate.  
